...
Скрипт использует встроенный «Планировщик заданий» (Task Scheduler) Windows для выполнения фоновых проверок каждые 5 минут. Программа не должна постоянно висеть открытой. Вы запускаете меню один раз, включаете бэкапы, и можете закрывать окно. Во время работы планировщик тихо запускает VBS-скрипт, который в скрытом режиме (без мелькания окон) проверяет, запущен ли процесс игры (`G1R` или `Gothic`). Если игра работает — папка с сохранениями копируется. Если игра выключена — скрипт просто завершает работу.

The script uses the built-in Windows Task Scheduler to run background checks every 5 minutes. The program does not need to stay open. You launch the menu once, start the backups, and you can close the window. During operation, the scheduler quietly runs a VBS script that silently (without window flickering) checks if the game process (`G1R` or `Gothic`) is running. If the game is running, the save folder is copied. If the game is closed, the script simply exits.
...
3. Запустите файл `Run.bat` (или сам скрипт `.ps1`, если используете PowerShell напрямую).
4. В появившемся меню выберите пункт **1**, чтобы установить фоновую задачу.
5. Закройте окно и играйте.
...
2. Extract the downloaded archive to any convenient location.
3. Run the `Run.bat` file (or the `.ps1` script itself if using PowerShell directly).
4. In the menu that appears, select option **1** to install the background task.
5. Close the window and play the game.
...
**Папка бэкапов по умолчанию:** `%USERPROFILE%\Documents\Gothic_Permadeath_Backups`
**Оригинальные сохранения игры:** `%LOCALAPPDATA%\G1R\Saved\SaveGames`
**Служебные файлы скрипта (Worker & VBS):** `%LOCALAPPDATA%\GothicBackup`

**Default backup folder:** `%USERPROFILE%\Documents\Gothic_Permadeath_Backups`
**Original game saves:** `%LOCALAPPDATA%\G1R\Saved\SaveGames`
**Script helper files:** `%LOCALAPPDATA%\GothicBackup`
...
1. Откройте меню программы и выберите **2. Stop backups**.
2. Удалите папку с бэкапами (в Документах).
3. Удалите папку со служебными файлами: `%LOCALAPPDATA%\GothicBackup`.

If you uninstall the game, the script and backups **will remain on your computer**. To completely remove the program:
1. Open the program menu and select **2. Stop backups**.
2. Delete the backup folder (in Documents).
3. Delete the helper files folder: `%LOCALAPPDATA%\GothicBackup`.
